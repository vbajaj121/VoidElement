...
export function CinematicProcessClient({ content }: { content: ProcessContent }) {
  ...
  // One IntersectionObserver, scoped to the horizontal track as its root, is
  // what drives the active-step caption/pagination below — this replaces the
  // GSAP ScrollTrigger pin-and-scrub that used to run desktop-only. A single
  // interaction model (swipe/scroll the filmstrip) now works identically at
  // every breakpoint instead of forking into a separate mobile stacked list.
  useEffect(() => {
  ...
  return (
    <div className="bg-background relative w-full overflow-hidden py-24">
      <Container>
      ...
      <div
        ref={trackRef}
        className="mt-12 flex snap-x snap-mandatory gap-4 overflow-x-auto px-6 pb-2 [scrollbar-width:none] sm:gap-6 sm:px-[max(1.5rem,calc((100vw-72rem)/2+1.5rem))] [&::-webkit-scrollbar]:hidden"
      >
        {steps.map((step, i) => (
          <div
            key={step.number}
          ...
          >
            ...
            >
              <ProcessStepArt colors={FALLBACK_GRADIENTS[i % FALLBACK_GRADIENTS.length]} image={step.imageUrl} />
            </m.div>
          </div>
        ))}
