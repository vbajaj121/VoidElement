...
import { uploadContentVideo } from "@/app/(admin)/admin/content/actions"
...
export function VideoUploadField({
  ...
  value,
  onChange,
  uploadAction = uploadContentVideo,
}: {
  label: string
  value: string | null
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.error("Upload failed. The file may be too large — try a shorter clip.")
    } finally {
      setUploading(false)
    }
  }
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-4">
        {value ? (
          <video src={value} muted playsInline className="border-border size-24 rounded-lg border object-cover" />
        ) : (
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            {uploading ? "Uploading…" : value ? "Replace" : "Upload video"}
          </Button>
          {value && (
            <Button type="button" variant="ghost" size="sm" data-cursor="hover" onClick={() => onChange(null)}>
